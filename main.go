...
func main() {
	...
	r.Get("/api/v2/quote/{route}/{type}", func(w http.ResponseWriter, r *http.Request) {
		...
		exists, err := quotes.TypeExists(r.Context(), typeParam)
		if err != nil {
			w.WriteHeader(500)
			w.Header().Set("Content-Type", "application/json")

			if os.Getenv("DEBUG") == "true" {
				json.NewEncoder(w).Encode(ServerErrorResponse(err.Error()))
			} else {
				json.NewEncoder(w).Encode(ServerErrorResponse("error checking type existence"))
			}
		...
		if routeParam == "random" {
			quote, err := quotes.GetRandomQuoteByType(r.Context(), typeParam)
			if err != nil {
				w.WriteHeader(500)
				w.Header().Set("Content-Type", "application/json")
				...
				if os.Getenv("DEBUG") == "true" {
					...
					json.NewEncoder(w).Encode(ServerErrorResponse("error fetching quote"))
				}
				return
			}
			...
			quote, err := quotes.GetLatestQuoteByType(r.Context(), typeParam)
			if err != nil {
				w.WriteHeader(500)
				w.Header().Set("Content-Type", "application/json")
